...
/>
  ...
  <van-tab title="全部完成">
    ...
    <div class="btox" v-for="(item,index) in list" :key="index"  v-show="val==1">
      <div class="btox_top">
        <span>{{item.dbh}}</span>
  ...
  <van-tab title="已完成">
        ...
        <div class="btox" v-for="(item,index) in list" :key="index" v-show="item.jgx==1">
      <div class="btox_top">
        <span>{{item.dbh}}</span>
        <span style="color: #ea7a2f;">交易完成</span>
      </div>
  ...
  data() {
    return {
      ...
      list:[
        ...
        {
          dbh:"订单编号：2020092915543981554",
          title:"自主每时每课特级教师-自主招生冲刺讲座6-多元方程组与可转化为多元方程组问题",
          sj:"时间：2020.09.29 15:54",
          jgx:1,
          but:"加入完成"
        },
