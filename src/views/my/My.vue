  ...
  <div>
    <van-overlay :show="show" @click="show = false">
      <div class="block">
        <img
          class="block11"
          src="https://msmk2019.oss-cn-shanghai.aliyuncs.com/uploads/image/2020zXdvqyxz1n1601360479.png"
          alt=""
        />
      </div>
    </van-overlay>
      ...
      <div class="box_bottom">
        <div class="login">
          ...
          <div class="login_right">
            <div v-show="username == ''">
              <span @click="add('/Register')">登录/注册</span>
            </div>
            <span>{{ username }}</span>
            <span class="xx"></span>
...
<script>
import { myusername, MySex } from "../api/api.js";
export default {
  data() {
    return {
      show: false,
  ...
  async mounted() {
    //获取当前页面的nick昵称名
    var a = await myusername();
    if (a.data.code == 200) {
      this.username = a.data.data.nickname;
      this.imgs = a.data.data.avatar;
      console.log(this.imgs);
    } else {
      // console.log(a.data.data.nickname);
      console.log(a.data.code);
      this.codes = a.data.code; //通过code值判断如果没有上传图片  就显示一个默认的图片
    }
    //再次请求user接口刷新下头像
    MySex({ avatar: this.imgs }).then((res) => {
      console.log(res);
    });
  },
  methods: {
...
.xx {
  margin-left: 0.15rem;
  background: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADwAAAA+CAYAAAB3NHh5AAAPbElEQ…F/F/SLvQQuqPLV303AqB9CPWQBL7OFGOnhz19sSoOvfr7+Pzk1XP97Kb4pAAAAAElFTkSuQmCC)
    50% no-repeat;
